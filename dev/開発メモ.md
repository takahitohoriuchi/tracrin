...
- [ ] ライセンスどうする？MIT？
  - ライセンスの表記とか
  - 貢献の分配とか
- [ ] git管理とSFTPでのデプロイの連携のしかたを決める
  - gitである程度固まって、ブランチマージしたら、デプロイってのがふつう
  - デプロイは堀内がやればよい
  - 小方さんにgithubアカウント作成してもらう（もってなければ）
  - 小方さんをgithubの`tracrin`プロジェクトに招待
  - vscodeのGithub pull request拡張機能をつかってみたい
## TODO
（to fix）
- [x] SFTPのrenameについて確認（サーバ側で、ローカルからのファイルリネーム許可設定ある？）
- [x] ID列の左に余白がほしい（スクショ撮りづらい）
- [x] IDを01, 02, という表記にしてほしい。
- [x] 現在グレーのところ、全部白にせよ
- [x] Speakerの列をもう少し細くしたい（現在余白が広くてムダ）
  - Speakerは、ふつう多くて２文字である。
- [ ] 同じスピーカーが連続して発話してるときに、スピーカーラベルを連続して書くかどうか
- [ ] spanタグの位置は、「絶対位置」で横着していて読みづらいのを、ローカル位置にしたい
- [ ] miraikan2.txtのデーアtが、出力後の551行目あたりの'['がおかしい
(to extend)
- [ ] ドラッグで、複数行の選択をできるようにする
- [ ] 動画の縦横サイズを調整できるようにする
- [ ] 部分選択している状態では、「部分再生モード」
- [ ] 部分選択解除ボタン
- [ ] 部分選択されているとき、その「行」を色づけなりでわかるようにする
...
---
### エレメント <a id="用語"></a>
<details><summary><b>hatsuwaObj（発話）</b>（1発話ごとに1つ）</summary>
...
```javascript
{
    ...
    text: 'ああ、おはようござんす'。
    speaker: '山田'
    rowNum: 3; //描画時に何行になるのか？spanタグ生成時に追加するプロパティ
}
```
</details>

<!-- 発話グループ -->
<details><summary><b>hatsuwaGroup[ ]（発話グループ）</b>（複数発話ごとに1つ）</summary>
...
<details><summary><b>hatsuwaGroups[ ][ ]（発話グループ群）</b>（1txtファイルまるごとで1つ）</summary>
...
<details><summary><b>tagArray[ ]</b>（1発話ごとに1つ）</summary>
...
<details><summary><b>tagArrays[ ][ ]</b>（1発話グループごとに1つ）</summary>
<a id='tagArrays'></a>
...
```javascript
tagArrays = [[tag, tag, ... ], [tag, tag, ...], ...]
```

二重配列：「ある発話グループにおける、i番目の発話のj番目タグ」
※発話i内を複数tagにする切れ目は、`[`記号。

</details>

<details><summary><b>tagObj</b>（発話より細かい）</summary>
<a id="tagObj"></a>
...
<details><summary><b>spanタグ要素</b>（1tagObjより細かい）</summary>
<a id="spanTag"></a>

  - HTMLの`<span>`タグ要素
  - 描画する最小単位
  - わりと多くのケースで孫spanタグと子発話は一対一対応するが、
  - 子発話が途中でウィンドウ右端に達したケースのみ、そこを切れ目にして２つの孫spanタグに分割される（長ーーい子発話からは３つ以上の孫spanタグが生まれることも！）
  - 以下２つのclass属性をもつ
    -  `i-j-k`（発話グループ`i`の発話`j`の子発話`k`から生まれたってこと。<span style="color:red">同じ子発話から生まれた孫spanたちは同じ`i-j-k`クラスをもつ！</span>）
    -  `tag`
...
<details>
<summary>
<b>hatsuwaTagSpans[ ][ ][ ]</b>（1txtファイルまるごとで1つ）
...
3. 各`tag`に`bracketID`フィールドを付与
   - <span style="color:red"><b>bracketID</b>...そのtagがふくむ`[`は、発話グループ内で何番目の被りタイミングを示しているか？</span>
...
「くくり記号」の扱い
ヒラキカッコの直前に、"<", ">", "°"がくることはあり得る？
"<'と">"の扱いに困る。。。
もともとタグが、急ぎなのかゆっくりなのかを判定しとかねばならぬ。

【win幅が10文字の場合】
発話：んいやあ<[もうほんと>おはよう[ございます。
↓
(1) splitして「発話タグ」を生成
んいやあ<, [もうほんと>おはよう, [ございます。

(2)「発話タグ」をwih幅にあわせて適宜分割して
「発話タグspan」を生成し、それぞれspanの描画X位置を決定

んいやあ<, [もうほんと>, [おはよ, うございます。

ここで同時に、spanの「発話記号」の位置チェックを行う！！！！
たとえば２コめの[もうほんと> の">"が行末にきちゃったら、
さらにまたspanを分割する！">"だけspanを分けて、改行！！！
んいやあ<, [もうほんと
>[おはよう, ございます。
